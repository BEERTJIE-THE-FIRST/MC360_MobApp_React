...
import { View, TextInput, TextInputProps, TextInputKeyPressEventData } from "react-native";

interface OTPOrPasscodeInputProps {
    setCodeInputs: React.Dispatch<React.SetStateAction<string[]>>
}

export default function OTPOrPasscodeInput({  setCodeInputs }: OTPOrPasscodeInputProps) {
    const [codes, setCodes] = useState<string[]>(["", "", "", "", ""]);
    const inputsRefs = useRef<TextInput[]>([]);

    useEffect(() => {
        // Initialize the array with refs for each input
        inputsRefs.current = Array.from({ length: 5 }, (_, i) => inputsRefs.current[i] || createRef<TextInput>());
    }, []);
